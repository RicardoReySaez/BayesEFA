...
While the package supports both `rstan` and `cmdstanr`, `rstan` is used as the default backend. For users who prefer the latest Stan features and faster estimation, `cmdstanr` can be installed by following the [official guide](https://mc-stan.org/cmdstanr/articles/cmdstanr.html).
